...
tmp <- ScaleData(so_epi, features =  genes_to_plot)
Idents(tmp) <- "harmony_cell_type_cluster"
mat <- AverageExpression(tmp, assays = "RNA", slot = "scale.data")$RNA
mat[mat < -2.5] <- -2.5
mat[mat > 2.5] <- 2.5
hm <- Heatmap(t(mat[genes_to_plot, cell_type_order]),
              name = "Z-scores",
              col = viridis::magma(256),
              cluster_rows = FALSE,
...
pdf(file.path(fig_dir,
              "cell_types_no_immune_refined_marker_heatmap_small.pdf"),
    height = 3,
    width = 9)
draw(hm)
a <- dev.off(); rm(a)
...
```{r}
p <- plot_harmony(so, "Ly6i", sorted = "random")
save_plot(file.path(fig_dir, "ly6i_harmony_umap_all_cells.pdf"), p, base_asp = 1.3)
p <- plot_harmony(so, "Ly6i", sorted = "random", group = "treatment")
save_plot(file.path(fig_dir, "ly6i_harmony_umap_all_cells_split.pdf"), p, base_asp = 1, nrow = 1, ncol = 2)
```
...
p <- plotDots(sce, 
  ...
  labs(x = NULL, y = NULL) + 
   theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
save_plot(file.path(fig_dir, "selected_marker_dotplot_v2.pdf"),
          p,  
         # base_height = 5,
          base_asp = 1.3)
p

p$data %>% write_csv(file.path(fig_dir, "selected_marker_dotplot_v2.csv"))
```
...
write10xCounts("geo", assays(sce)$counts, overwrite = TRUE, version="3")

mdata <- as.data.frame(colData(sce))
mdata <- cbind(cell = rownames(mdata), mdata)
mdata <- mdata[c("cell", "harmony_cell_type_cluster", "sample", "expt", "treatment")]
mdata <- cbind(mdata, reducedDim(sce, "HARMONY_UMAP"))
write_csv(mdata, "geo/cell_metadata.csv.gz")
```
