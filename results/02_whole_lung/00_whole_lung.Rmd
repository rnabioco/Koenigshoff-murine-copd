...
In this experiment, there are 3 sample per treatment, captured using the 10X Genomics 3’ Platform and subsequent sequencing.
...
Suggest using some minor filters to remove cells with outlier mitochondrial UMIs (>3 MAD or > 20%), > 50K UMI or < 1000 UMIs, > 5000 or < 1000 genes detected . 
...
cur_count <- dplyr::count(so@meta.data, sample)
filter_summary$"Too high # of genes" <- cur_count %>%
  left_join(prev_count, by = "sample") %>% 
  mutate(n = n.y - n.x) %>% 
  dplyr::select(sample, n)
prev_count <- cur_count

so <- subset(so, subset = nFeature_RNA > 750)
cur_count <- dplyr::count(so@meta.data, sample)
...
```{r}
so <- qread("objects/so_v2.qs", nthreads = 4)
```
...
Using these reference datasets as a guide, each cluster will be assigned to the most correlated cell type in the reference. Most clusters have very high correlation to the cell types in the reference lung dataset. **After additional QC, there are now populations that have high > 0.7 correlation for club cells** 
...
Next I'll also try correlating against the reference per cell rather than per cluster. Shown below are the correlation cooefficients computed per cell for each indicated cell type from the reference Strunz et al dataset.
...
The club cells are expected to be rare and based on marker gene expression there is no enrichment for club cell markers in any large cluster. `Scgb1a1` is broadly expressed, but only a small subset seems to express very highly. There is 1 cluster that matches ciliated cells with high club cell markers. 
...
hm
```
Shown below are the highest correlated cell type for each cluster. Note that the clusters with highest correlation to club cells have generally low correlation values and are likely spurious correlations. 
...
There are two  club cell clusters, annotated as goblet and club cells. 

```{r}
club_cell_types <- str_subset(unique(so$coarse_cell_type), "[Cc]lub|[Gg]oblet")
...
The goblet and club cells will next be reclustered separately. 
```{r, eval = FALSE}
club_so <- subset(so, subset = coarse_cell_type %in% club_cell_types)
...
```{r}
club_so <- qread("objects/club_so_v2.qs", nthreads = 4)
plot_umap(club_so, "seurat_clusters", group = "sample", pt_size = 0.5)
```

Note there is cluster (#3) that is only found in the E2 sample. 
...
```{r}
library(scran)
sce <- DietSeurat(club_so) %>% as.SingleCellExperiment()
m.out <- findMarkers(sce, 
                     groups = sce$seurat_clusters,
                     block=sce$sample,
                     direction="up") 

topx <- map(as.list(m.out), ~.x %>% as.data.frame() %>% 
              filter(Top < 10) %>% 
...
cell_props <- cell_props[, to_plot, drop = F]
...
Based on these re-analyses it is now more clear that there are club cells confidently identified in this dataset, however their are not very many of these cells (462). 

```{r, eval = FALSE}
qsave(so, "objects/so_v2.qs")
qsave(club_so, "objects/club_so_v2.qs")
```
