...
```{r}
clusters <- unique(so@meta.data$coarse_cell_type)
treatment <- c("Elastase", "Saline")
so_tmp <- so
so_tmp$cluster_treatment <- str_c(so$treatment, "_", so$coarse_cell_type)
...
write_markers_xlsx(de_sig_res, 
                     file.path("tables",
                               "whole_lung_all_cells_Elastase_v_Saline_per_cell_type_top200_significant.xlsx"),
