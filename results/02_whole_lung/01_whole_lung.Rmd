...
```
```{r}
so_at2 <- FindClusters(so_at2, 
             res = c(1.1, 1.3, 1.5), 
               random.seed = seed_value)

plot_umap(so_at2, str_c("RNA_snn_res.", c(1.1, 1.3, 1.5)), group = "treatment")
```

```{r}
mkrs <- get_marker_summaries(so_at2, "RNA_snn_res.1.5", outdir = mkrs_dir, prefix = "whole_lung_at2_further_refined")
...
tmp <- sumCountsAcrossCells(lung_sce, 
                            DataFrame(sample = lung_sce$sample,
                                      treatment = lung_sce$treatment,
                                      cluster = lung_sce$RNA_snn_res.1.5))
...
m.alt <- findMarkers(lung_sce,
                     groups = lung_sce$RNA_snn_res.1.5, 
                     design=design, 
                     direction="up", 
                     pval.type = 'some')
...
openxlsx::write.xlsx(c(README = list(scran_marker_description), 
                       de_res), 
                     file.path("tables", "whole_lung_at2_cells_Elastase_v_Saline_per_further_refined_clustering_edgeR.xlsx"),
                     overwrite = TRUE)
...
openxlsx::write.xlsx(c(README = list(scran_marker_description2), 
                       de_res), 
                     file.path("tables", "whole_lung_at2_cells_per_further_refined_clustering_ttest.xlsx"),
                     overwrite = TRUE)

de_df <- bind_rows(de_res, .id = "group")
write_tsv(de_df, "markers/whole_lung_at2_further_recluster_scran_markers.tsv")
```


```{r}
qsave(so_at2, "objects/so_at2.qs")
#so_at2 <- qread("objects/so_at2.qs")
```
